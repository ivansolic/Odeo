...
- Whole-lifecycle workflow: discovery → PRD → design → `/build` → review →
  `/merge` → `/outcome` → `/learn`.
...
- Compounding knowledge: `/learn`, `/knowledge-refresh`, the `knowledge/` base.
...
- Central learning, light: `/contribute-lesson` (opt-in, sanitized, approval-gated)
...
- **Output language**: choose the language the system writes human-facing prose in
  ...
  Shipped: `/language` plus a guardrail that keeps commit subjects, identifiers and
...
- Smoother `/outcome` signal ingestion (analytics connectors).

## Later / exploring

- **Skill evolution from lessons**: (1) a curated pipeline where community lessons
  and rubric improvements feed `/improve` into shipped skills; (2) an opt-in
...
- Contribute a lesson with `/contribute-lesson`.
