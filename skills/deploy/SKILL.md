...
smoke test after. `/ci` verifies; `/deploy` ships. It closes the gap before
`/outcome` (which needs something actually shipped).
...
not permission, shipping to production is the human floor (like `/merge`).

## Preconditions
- You are on the release ref (usually `main` after `/merge`, or a ref you name),
  the working tree is clean.
- Tests + typecheck are green HERE, and CI is green if it exists (check
  `.github/workflows/`; `/ci` is what generates it). A suite that has never run
  is not green.
...
6. **Record it.** Append the deploy to `docs/deploys/history.md` (date, env, ref,
   ...
   `/start` and `/outcome` can see what shipped and when. Committed, it is the
   ship history, so it never carries a credential.
7. **Close.** Confirm what is live where. Offer `/outcome` "in about a week, once
   there is real usage data", the loop that checks the change actually worked.

## Worked example
"/deploy" (a web app whose CLAUDE.md says Deployment: Vercel)
...
- Then "/deploy to production" -> STOPS for your explicit yes; on yes, same steps
