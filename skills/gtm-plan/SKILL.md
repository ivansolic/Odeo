---
description: Build a focused go-to-market PLAN, who you're launching to, the message, the channels, the motion, and how you'll know it worked. Use ahead of a launch or a new segment push. The single GTM artifact; the /go-to-market orchestrator runs the whole launch phase around it.
...
beachhead, not a blast to everyone. Pulls together `/market-segments`, `/positioning`,
`/value-proposition`, and `/metrics`. (This is the plan *artifact*; the `/go-to-market`
...
## Process
1. **Target:** the specific beachhead/ICP for this launch (from `/market-segments`). Not "everyone."
2. **Message:** the positioning + value prop translated into the words this segment uses
   (from `/positioning`, `/value-proposition`). One core promise.
...
7. **Success metrics + guardrails:** what "a good launch" means in numbers (links to
   `/metrics`), and the leading signals you'll watch in week 1.

## Output
A focused GTM plan saved under `docs/`. After launch, judge it with `/outcome`.
