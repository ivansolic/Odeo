...
- `/build` also runs this automatically before dispatching parallel work.

## Process (read-only)
1. **Read the order:** `docs/stories/` (the `USR-NNN` dependency order from `/stories`) + the roadmap in `docs/` (from `/plan`). What's unblocked, what comes first.
...
5. **State the limit honestly:** "based on the files touched so far plus an estimate, not a guarantee; `/merge` is the final safety net (it surfaces any real conflict)."
...
## Output
A recommendation + the reasoning, in chat. It does **not** branch, edit, or build. You decide, then run `/build`.
...
- Always state the heuristic limit + that `/merge` is the backstop.
